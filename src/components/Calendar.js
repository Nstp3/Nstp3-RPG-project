...
// ============================================================
// components/Calendar.js — календарь на +1 год вперёд
// Показывает задачи по датам (история + запланированные)
// ============================================================
...
export function renderCalendar() {
  ...
  // Собираем задачи из истории по этому месяцу
  const historyMap = {};
  ...
  for (let d = 1; d <= daysCount; d++) {
    ...
    cells.push(`
      <div class="cal-day ${isToday ? 'cal-day--today' : ''} ${hasTasks ? 'cal-day--has' : ''}" data-day="${d}" style="
        position:relative;
  ...
  return `
    <div class="card">
      <div class="card-title" style="display:flex;justify-content:space-between;align-items:center;">
        <button class="cal-nav" id="calPrev" style="background:none;border:none;color:var(--text2);cursor:pointer;font-size:16px;${!canPrev?'opacity:0.3;pointer-events:none;':''}">‹</button>
        <span style="font-size:13px;font-weight:600;text-transform:capitalize;">${monthName}</span>
        <button class="cal-nav" id="calNext" style="background:none;border:none;color:var(--text2);cursor:pointer;font-size:16px;${!canNext?'opacity:0.3;pointer-events:none;':''}">›</button>
      </div>
...
export function bindCalendar() {
  ...
  document.getElementById('calGrid')?.addEventListener('click', e => {
    ...
    if (tasks.length === 0) {
      detail.innerHTML = `<p style="font-size:11px;color:var(--text2);">Нет задач за этот день</p>`;
    } else {
      const catColors = {
      ...
      detail.innerHTML = `
        <div style="font-size:11px;color:var(--text2);margin-bottom:6px;">${day} — выполнено ${tasks.length}</div>
        ${tasks.map(task => `
